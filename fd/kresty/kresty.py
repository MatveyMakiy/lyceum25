...
def krestikiNoliki():
    ...
    for i in range (3):
        line = input().strip()
        kletki = [cell.strip() for cell in clean.split() if cell.strip() != '']
        if len(kletki) != 3:
            print("Ошибка ввода: строка должна содержать ровно 3 клетки.")
            return
        if not all(c in ['X', '0', ' '] for c in kletki):
            print("Ошибка: клетки могут содержать только 'X', '0' или пробел.")
            return
