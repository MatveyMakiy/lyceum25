...
def find_next_after_pattern(player_history, target): # функция ищет, какой ход чаще всего следует после заданного паттерна
...
def classify_player(player_history, computer_history): # функция определяет тип игрока по его поведению в предыдущих раундах
...
def hardBotChoice(player_history, computer_history, round_number): # функция выбора хода для сложного уровня бота
    ...
    player_type = classify_player(player_history, computer_history) # определяем тип игрока: консерватор, адаптив и т.д.
    if len(player_history) >= 3: # если есть хотя бы 3 хода
        recent_pattern = tuple(player_history[-3:]) # последние 3 хода игрока
        predicted_next = find_next_after_pattern(player_history, recent_pattern) # предсказываем, какой ход будет следующим
